...
async function Defender() {
  ...
  if (app.isPackaged) {
    ...
    if (result.success) {
      console.log(logo, "Added Pecinhas to Windows Defender Exclusions")
    } else {
      console.error(logo, "Failed to add Pecinhas to Windows Defender Exclusions", result.error)
    }
  } else {
